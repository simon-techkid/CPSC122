...
References:
    GeeksForGeeks, Nov. 8, 2025, https://www.geeksforgeeks.org/cpp/operator-overloading-cpp/
*/
...
struct doubleNode {
    ...
    // Implement relational operators for sorting

    bool operator<(const doubleNode<itemType>& other) const {
...
public:
    ...
    post: The current object is an exact and deep copy of what lst points to.
    */
    ListD(ListD<itemType>* lst);
...
private:
    ...
    */
    doubleNode<itemType>* FindPosition(int pos);
    ...
    */
    ...
    // Helper functions to get default values for head and tail nodes
    itemType GetDefaultHead();

    // Helper functions to get default values for head and tail nodes
    itemType GetDefaultTail();

    int length;                   //length of the list
    doubleNode<itemType>* head;   //points to the first dummy node
    doubleNode<itemType>* tail;   //points to the last dummy node
};
